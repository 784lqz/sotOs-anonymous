/* sotOs · sotcron audit_ipc · γ.  IPC to orch's audit-receive EP. */
#include <stdint.h>
#include <stdio.h>
#include <sel4/sel4.h>
...
void sotos_audit_set_ep(uintptr_t ep_slot) {
    ...
    printf("[sotcron] audit_ep set · slot=%lu\n", (unsigned long)g_audit_ep);
}
...
void sotos_audit_emit(uint16_t kind, uint32_t slot, uint64_t arg0) {
    if (g_audit_ep == 0) {
        printf("[sotcron] AUDIT kind=0x%02x slot=%u arg0=0x%lx (NO_EP)\n",
    ...
    printf("[sotcron] AUDIT kind=0x%02x slot=%u arg0=0x%lx\n",
           kind, slot, (unsigned long)arg0);
}
